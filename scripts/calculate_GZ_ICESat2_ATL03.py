...
def calculate_GZ_ICESat2(base_dir, FILE, VERBOSE=False, MODE=0o775):
    ...
    for gtx in sorted(IS2_atl03_beams):
        ...
        #-- ocean tide
        tide = np.zeros_like(val['heights']['h_ph'],dtype=np.int)
        #-- dynamic atmospheric correction
        dac = np.zeros_like(val['heights']['h_ph'],dtype=np.int)
        #-- geoid height
        geoid = np.zeros_like(val['heights']['h_ph'],dtype=np.int)
        ...
        for imin,imax in ice_gz_indices:
            ...
            X,Y = pyproj.transform(proj1, proj2, v['lon_ph'][i], v['lat_ph'][i])
