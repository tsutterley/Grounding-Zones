...
Program to sync Reference Elevation Map of Antarctica (REMA) DEM tar files
...
def pgc_rema_sync(base_dir, VERSION, RESOLUTION, TILES=None,
	...
	remote_dir = posixpath.join(HOST,'REMA','mosaic',VERSION,RESOLUTION)
	...
	remote_dir = posixpath.join(HOST,'REMA','indexes')
	#-- open connection with PGC server at remote directory
	request = urllib2.Request(remote_dir)
	response = urllib2.urlopen(request, timeout=20)
	#-- read and parse request for files (names and modified dates)
...
def usage():
	...
	print(' -V X, --version=X\tREMA DEM version')
	print('\tv1.0\n\tv1.1')
	print(' -R X, --resolution=X\tREMA DEM spatial resolution')
	print('\t1km\n\t200m\n\t100m\n\t8m')
	print(' -T X, --tile=X\t\tREMA DEM tiles to sync')
...
def main():
	...
	if check_connection():
		pgc_rema_sync(base_dir, VERSION, RESOLUTION, TILES=TILES,
