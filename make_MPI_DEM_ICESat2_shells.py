#!/usr/bin/env python
u"""
make_MPI_DEM_ICESat2_shells.py
Written by Tyler Sutterley (07/2019)

Creates a shell script to interpolate a digital elevation model to compare with
	ICESat-2 data
...
#-- MPI child program
child_program = {}
child_program['ATL03'] = os.path.join(filepath,'MPI_DEM_ICESat2_ATL03.py')
child_program['ATL06'] = os.path.join(filepath,'MPI_DEM_ICESat2_ATL06.py')
...
def main():
	...
	long_options = ['help','np=','directory=','year=','product=','release=',
		'version=','granule=','track=','shell=','clobber','verbose','mode=',
		'header=','memory=','queue=','walltime=','mail-user=','mail-type=']
	optlist,arglist=getopt.getopt(sys.argv[1:],'hP:D:R:Y:S:CVM:H:',long_options)
	...
	output_shell_script = "interp_{0}_DEM_{1}.sh".format(PRODUCT,today)
	...
	for sd in sorted(subdirectories):
		...
		for FILE in FILES:
			...
			args=(mpi_prefix,child_program[PRODUCT],verbose_flag,mode_flag,FILE)
			print("{0} {1} {2}{3} {4}".format(*args), file=f)
	#-- close the shell script and change permissions to MODE
	f.close()
	os.chmod(os.path.join(filepath,output_shell_script), MODE)

#-- PURPOSE: find ICESat-2 HDF5 files
def find_ICESat2_files(base_dir, SUBDIRECTORY, PRODUCT, RELEASE, VERSIONS,
	...
	GRANULES = [3,4,5,10,11,12] if not np.any(GRANULES) else GRANULES
	...
	for f1 in input_files:
		...
		#-- set hemisphere flag based on granule
		HEM = set_hemisphere(GRAN)
		#-- check if there is an output file for the input file
		f2 = file_format.format(PRODUCT,DEM_MODEL[HEM],YY,MM,DD,HH,MN,SS,
...
def print_slurm_header(fid, Y, M, D, PRODUCT, RELEASE, PROCESSES,
	...
	#-- print job name
	args = (PRODUCT,Y,M,D)
	print("#SBATCH --job-name=dem_{0}_{1}-{2}-{3}".format(*args),file=fid)
...
def print_pbs_header(f, Y, M, D, PRODUCT, RELEASE, PROCESSES, MEMORY,
	...
	#-- print job name
	args = (PRODUCT,Y,M,D)
	print("#PBS -N dem_{0}_{1}-{2}-{3}".format(*args),file=f)
	#-- print mail-user and mail-type commands
	if MAIL_USER:
